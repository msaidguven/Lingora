...
export default function Header({ currentScreen, onNavigate, userLevel, quizType = null }) {
  ...
  // Kullanıcı ve tema bilgisini yükle
  useEffect(() => {
    ...
    // Sistem veya local storage teması kontrolü
    const savedTheme = localStorage.getItem("theme") || "dark";
    setTheme(savedTheme);
    document.documentElement.setAttribute("data-theme", savedTheme);
  }, []);

  // Dışarı tıklayınca menüyü kapatma mekanizması
  useEffect(() => {
  ...
  return (
    <div style={styles.wrapper} className="header-wrapper">
      ...
      <div style={styles.bar}>
        ...
        {/* Sağ Üst Açılır Menü (Profil / Ayarlar / Tema) */}
...
const css = `
  ...
  .nav-btn-not-active { color: var(--text-muted) !important; }
  ...
  .dropdown-item { color: var(--text-main) !important; transition: background 0.2s; }
